...
const createProgressForUser = async (userId: string, courseId: string) => {
...
const getProgressSingleCourse = async (userId: string, courseId: string) => {
    const course = await Course.findById(courseId);
    ...
    const completedLessonsInCourse = progress.modules.reduce(
        (acc, m) => acc + (m.completedLessons?.length || 0),
        0
    );

    const pendingLessonsInCourse =
        totalLessonsInCourse - completedLessonsInCourse;
...
const completeLesson = async (userId: string, courseId: string, lessonId: string) => {
    ...
    const moduleData = course.modules.find((m) =>
        m.lessons?.some((l) => l._id?.toString() === lessonId)
    );

    if (!moduleData || !moduleData._id) throw new Error("Module not found");

    const userModule = progress.modules.find(
        (m) => m.moduleId.toString() === moduleData._id!.toString()
    );

    if (!userModule) throw new Error("User module progress not found");

    // Add lesson if not already completed
    if (!userModule.completedLessons.some((id) => id.toString() === lessonId)) {
        userModule.completedLessons.push(lessonId as any);
    }
...
export const UserCourseProgressServices = {
