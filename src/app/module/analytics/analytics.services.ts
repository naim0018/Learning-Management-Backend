...
import { Enroll } from "../CourseEnroll/enroll.model";
...
const adminAnalytics = async () => {
    ...
    const totalActiveCourse = await Course.find({ courseStatus: "Published" }).countDocuments();
    const totalCourse = await Course.countDocuments();
    const activeLerner = await User.find({ isActive: true }).countDocuments();
    ...
    const weeklyEnrollments = await Enroll.aggregate([
        { $match: { createdAt: { $gte: last8WeeksDate } } },
        {
            $group: {
                _id: {
    ...
    for (let i = 7; i >= 0; i--) {
        ...
        const userData = weeklyUsers.find(
            w => w._id.year === year && w._id.week === week
        );

        const enrollmentData = weeklyEnrollments.find(
            e => e._id.year === year && e._id.week === week
        );
    ...
    const topSellingCourses = await Enroll.aggregate([
        {
            $group: {
                _id: "$courseId",
    ...
    const results = await Enroll.aggregate([
        {
            $match: {
...
const userAnalyticsData = async (userId: Types.ObjectId) => {
    // 1️⃣ Find the user
    ...
    const userCourseIds =
        findUser.enrollCourse?.map((c) => c.courseId?.toString()) || [];
    ...
    const courses = await Course.find({ _id: { $in: userCourseIds } });

    // 4️⃣ Fetch user progress for all courses
    const progressList = await UserCourseProgress.find({
        userId,
    ...
    const coursesWithProgress = courses.map((course) => {
        ...
        const completedLessons =
            progress && Array.isArray(progress.modules)
